#!/usr/bin/env python3
"""assemble_html2.py — v2 of assemble_html.py: builds dist/doom_cassette2.html
from the doom1-mode engine pack (pack/doom_pack1.*) + freedoom_trim.wad
(real Freedoom E1 maps, trimmed; see trim_freedoom.py).

v1 (dist/doom_cassette.html, miniwad) is left untouched as the fallback.
...
# Optional argv[1] overrides the output path (default: dist/doom_cassette2.html).
OUT = sys.argv[1] if len(sys.argv) > 1 else os.path.join(DIST, "doom_cassette2.html")

engine_js = open(os.path.join(HERE, "pack", "doom_pack1.js"), "rb").read()
wasm = open(os.path.join(HERE, "pack", "doom_pack1.wasm"), "rb").read()
wad = open(os.path.join(HERE, "freedoom_trim.wad"), "rb").read()
...
carrier) was stored as ~9 minutes-per-megabyte of audio on one side of a C90
...
Game data: trimmed from Freedoom: Phase 1 v0.13.0 (https://freedoom.github.io/),
  maps E1M1 + E1M5 plus a budgeted subset of its textures, flats and sprites
  (front-rotation-only, Jaguar-DOOM style). BSD 3-clause license, reproduced
...
splash = (b'<div id="ov">'
          b'<div id="cass"><b>SIDE A &middot; DOOM &middot; 90 min</b><i class="l"></i>'
          b'<i class="r"></i></div>'
          b'<h1>DOOM</h1>'
          ...
          b'game data: Freedoom Phase 1 (BSD), trimmed &middot; '
...
print("lzma9 total: %.1f KB" % (len(lzma.compress(html, preset=9)) / 1024.0))

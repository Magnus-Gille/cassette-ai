#!/usr/bin/env python3
"""
bside_techno — a techno/IDM remix of the cassette-ai master10 data transfer.

Every musical element is derived from the REAL signal:
  * TEMPO     : exactly the d2x frame period (0.983333 s = 2 beats -> 122.034 BPM).
  * KICK      : the real frame-preamble chirp (800->3200 Hz, 0.25 s) sampled from the
                capture, pitched down 8x (-> 100->400 Hz thump) + its own HF click.
  * HATS      : sliced HF phase-hop transients from the real d2x DQPSK capture (>6 kHz).
  * BASS      : square-ish additive synth on sub-octaves of the REAL 375 Hz carrier grid
                (F#2 = 750/8, A#2 = 1875/16, C#3 = 1125/8, E3 = 2625/16), filter envelope,
                16-step pattern.
  * STABS/PAD : DQPSK-style phase-hopping resynthesis on the exact carrier grid
                (750/1125/1500/1875/2250/2625 Hz).
  * BED       : the REAL capture (d2x + N512 sections), band-passed 2-6 kHz, sidechain-
                ducked under the kick.
  * RISERS    : resynthesized global chirp (500->5000 Hz linear, per manifest) stretched
                to 2 bars; the REAL up-chirp sample as a zap on drop downbeats; the REAL
                down-chirp closes the piece.
  * CRASH/CLAP: the REAL Schroeder multitone sounder splash.
  * TICKS     : the REAL preamble samples placed at the exact frame period (every 2 beats).

Form: intro(8) build(8) drop1(16) break(8) drop2(16) outro(8) bars @ 122.034 BPM ~= 126 s.

Output: stereo 48 kHz 16-bit, peak <= -1 dBFS, RMS ~ -15 dBFS, 2 s fade-in / 4 s fade-out.
Seed: 20260612 (logged below).
"""
import json
import numpy as np
...
OUT = f"{BASE}/bside_remix/bside_techno.wav"

# ---------------------------------------------------------------- timing grid
FRAME_S = 0.983333          # d2x N256_sp2 nominal frame period (palette.json)
BEAT = FRAME_S / 2.0        # -> 122.034 BPM
BAR = 4 * BEAT
STEP = BAR / 16.0
N_BARS = 64
TAIL_S = 2.6
...
def add_at(buf, t_s, x, pan=0.0, gain=1.0):
    """Mix mono x into stereo buf at time t_s with constant-power pan in [-1,1]."""
    i0 = int(round(t_s * SR))
    if i0 >= len(buf):
        return
...
print(f"[load] capture {len(cap)/SR:.1f} s @ {SR} Hz   seed={SEED}")

def cslice(t0, dur):
    return cap[int(t0 * SR): int((t0 + dur) * SR)].copy()

# ---------------------------------------------------------------- instruments
# KICK: real preamble chirp (capture 46.8634 s) pitched down 8x -> 100->400 Hz thump.
pre_n512 = cslice(46.8634, 0.25)
pre_d2x = cslice(205.3075, 0.25)
...
kick = np.tanh(3.0 * norm_peak(body) * kenv)
...
# HATS: HF transient grains sliced from the real d2x section.
hsrc = hp(cslice(208.0, 30.0), 6000, 6)
e = np.convolve(hsrc ** 2, np.ones(256) / 256, mode='same')
pk, props = sig.find_peaks(e, distance=int(0.05 * SR))
order = np.argsort(e[pk])[::-1][:96]   # top-energy phase-hop moments (HF is near-uniform)
grain_pos = pk[order]
assert len(grain_pos) >= 32
print(f"[hats] {len(grain_pos)} transient grains sliced from real capture")

def hat(open_=False):
    pos = int(rng.choice(grain_pos))
    dur = 0.16 if open_ else 0.045
    tau = 0.060 if open_ else 0.016
    n = int(dur * SR)
...
def crash_pair():
    cl = hp(cslice(26.15, 2.2), 1200, 4)
    cr = hp(cslice(26.161, 2.2), 1200, 4)
    n = min(len(cl), len(cr))
    env = np.exp(-np.arange(n) / (0.75 * SR)) * np.minimum(np.arange(n) / (0.004 * SR), 1.0)
...
BASS_PAT = [  # (step, freq, accent, dur_s)
...
def dqpsk_stab(freqs, amps, dur, sym_s=FRAME_S / 16):
    ...
    for f, a in zip(freqs, amps):
        ph = rng.integers(0, 4, size=sym[-1] + 1) * (np.pi / 2)
...
# RISER: resynth of the global chirp (500->5000 Hz linear) over 2 bars + real noisefloor.
def riser(dur):
    n = int(dur * SR)
    t = np.arange(n) / SR
    f = 500.0 + (5000.0 - 500.0) * t / dur
    chirp = np.sin(2 * np.pi * np.cumsum(f) / SR)
    nz = hp(tile_xfade(cslice(43.0, 2.9), n, 0.2), 2500, 4)
    nz = norm_peak(nz)
...
kick_bars = list(range(4, 32)) + list(range(40, 60))
kick_times = [t_of(b, s) for b in kick_bars for s in (0, 4, 8, 12)]
for tks in kick_times:
    add_at(kick_tr, tks, kick, pan=0.0, gain=0.88)
    i0 = int(tks * SR)
    n = min(int(0.5 * SR), N - i0)
    if n > 0:
        curve = np.exp(-np.arange(n) / (0.15 * SR))
        duck[i0:i0 + n] = np.maximum(duck[i0:i0 + n], curve)
print(f"[kick] {len(kick_times)} hits, 4-on-floor @ {60/BEAT:.2f} BPM")

# bass
for b in BUILD:
    prog = (b - 8) / 7.0
    for (st, f0, acc, dur) in [(2, F2, 1.0, 0.21), (6, F2, 0.9, 0.21),
                               (10, F2, 0.95, 0.21), (14, F2, 0.85, 0.21)]:
        nb = bass_note(f0, dur, 180 + 320 * prog, 500 * acc, acc)
        add_at(bass_tr, t_of(b, st), nb, 0.0, 0.36)
for b in DROP1:
    for (st, f0, acc, dur) in BASS_PAT:
        add_at(bass_tr, t_of(b, st), bass_note(f0, dur, 700, 1800 * acc, acc), 0.0, 0.36)
for b in DROP2:
    for (st, f0, acc, dur) in BASS_PAT2:
        add_at(bass_tr, t_of(b, st), bass_note(f0, dur, 800, 2200 * acc, acc), 0.0, 0.38)
for b in OUTRO:
    if b < 60:
        for (st, f0, acc, dur) in BASS_PAT:
            add_at(bass_tr, t_of(b, st), bass_note(f0, dur, 600, 1200 * acc, acc), 0.0, 0.33)

# hats / claps / ticks / stabs straight into mix
for b in range(2, N_BARS):
    if b in BREAK:
        steps = [10] if b % 2 else [2, 10]
    elif b in INTRO or (b in OUTRO and b >= 60):
        steps = [2, 6, 10, 14] if b >= 4 else [2, 10]
    else:
        steps = [2, 6, 10, 14]
    in_drop = (b in DROP1) or (b in DROP2)
    for st in steps:
        open_ = in_drop and st == 10
        g = hat(open_)
        pan = 0.3 if (st // 4) % 2 else -0.3
        add_at(mix, t_of(b, st), g, pan, 0.20 if open_ else 0.23)
    if b in DROP2:  # 16th ticking
...
tick_bars = list(INTRO) + list(BREAK) + list(OUTRO)
ti = 0
for b in tick_bars:
...
# break: pad + gated REAL-capture data stabs
pl, pr = pad_pair(8 * BAR)
add_st(mix, t_of(32), pl, pr, 0.085)
gate_src = bp(cslice(212.0, 20.0), 600, 3000, 4)
gp = 0
for b in BREAK:
    for st in (0, 2, 4, 6, 8, 10, 12, 14):
        ...
        gp += int(0.123 * SR)
        add_at(mix, t_of(b, st), norm_peak(g), 0.2, 0.16)

# risers + zaps + crashes
add_at(mix, t_of(14), riser(2 * BAR), 0.0, 0.30)
add_at(mix, t_of(38), riser(2 * BAR), 0.0, 0.30)
for bar in (16, 40):
    add_at(mix, t_of(bar), zap_up, 0.0, 0.30)
    add_st(mix, t_of(bar), crashL, crashR, 0.32)
add_st(mix, t_of(32), crashL, crashR, 0.20)
add_at(mix, t_of(62), zap_down, 0.0, 0.30)   # the tape says goodbye

# ---------------------------------------------------------------- modem bed
bed = np.zeros(N)
bed_plan = [  # (bar_start, bar_end, capture_src_t, gain)  N512 for calm, d2x for drops
    (0, 8, 48.0, 0.40), (8, 16, 206.0, 0.32), (16, 32, 206.0, 0.30),
    (32, 40, 52.0, 0.40), (40, 56, 215.0, 0.30), (56, 64.6, 60.0, 0.38),
]
for (b0, b1, src_t, g) in bed_plan:
    i0, i1 = int(t_of(b0) * SR), min(int(b1 * BAR * SR), N)
    seg = tile_xfade(cslice(src_t, min(30.0, 380.0 - src_t)), i1 - i0, 0.4)
...
for _ in range(40):
    ...
    if rms_db < target_rms_db:
        ...
        hi = g
final = np.tanh(g * mix) * (peak_lin / np.max(np.abs(np.tanh(g * mix))))
...
assert dur >= 90 and dur <= 150 and peak_db <= -1.0 and -16.5 < rms_db < -13.5
...
print(f"[QA] onsets: {len(op)} total -> {len(op)/dur:.2f}/s overall", end="")
d1 = (t_[op] >= t_of(16)) & (t_[op] < t_of(32))
print(f" | drop1 rate {d1.sum()/(16*BAR):.2f}/s")
print("[QA] per-section RMS (dBFS):")
for name, b0, b1 in (("intro", 0, 8), ("build", 8, 16), ("drop1", 16, 32),
                     ("break", 32, 40), ("drop2", 40, 56), ("outro", 56, 64)):
    seg = mono[int(t_of(b0) * SR):int(t_of(b1) * SR)]
    print(f"      {name:6s} {20*np.log10(np.sqrt(np.mean(seg**2))):+6.1f}")
print(f"[QA] seed={SEED}  bpm={60/BEAT:.3f}  bar={BAR:.4f}s  total_bars={N_BARS}")

...
OUT = f"{BASE}/bside_remix/sideB/alt/t5.wav"
...
def hat(open_=False):
    ...
    g = lp(hp(norm_peak(g), 4000, 4), 5500, 4)  # tamed: bandpass 4k–5.5k; removed 7.5k+ ultra-shrill lift
...
def dqpsk_stab(freqs, amps, dur, sym_s=FRAME_S / 16):
    ...
    return fade_edges(lp(y, 6000, 4) * env, 2.0)  # lowered stab LP 9500→6000 Hz to reduce HF buzz
...
for b in range(2, N_BARS):
    ...
    for st in steps:
        ...
        add_at(mix, t_of(b, st), g, pan, 0.14 if open_ else 0.16)  # -3.3 dB vs original 0.20/0.23
    # 16th-note ticking rolls in the heavier drops
    if (b in DROP2) or (b in DROP3):
        for st in (1, 3, 5, 7, 9, 11, 13, 15):
            add_at(mix, t_of(b, st), hat(False), 0.45 if st % 4 == 1 else -0.45, 0.08)  # -2.8 dB vs original 0.11

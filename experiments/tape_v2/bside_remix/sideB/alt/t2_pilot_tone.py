...
OUT = "/Users/magnus/repos/cassette-ai/experiments/tape_v2/bside_remix/sideB/alt/t2.wav"

cap, cap_sr = sf.read(CAPTURE, dtype="float64")
assert cap_sr == SR and cap.ndim == 1, "expected mono 48k capture"

PILOT = 2437.5    # one octave below the 13th harmonic (4875 Hz) -- less fatiguing
...
# soften the pilot: gentle lowpass so the sine cluster loses its sterile edge
pilot_lp_b, pilot_lp_a = sg.butter(3, 4500 / (SR / 2), "low")
pilotL = sg.lfilter(pilot_lp_b, pilot_lp_a, pilotL)
pilotR = sg.lfilter(pilot_lp_b, pilot_lp_a, pilotR)
pilot_amp = 0.22 * pilot_gate * pbreath   # -2.7 dB vs original 0.30
...
# tame shrill grid partials above ~5 kHz (the background whistle complaint)
grid_lp_b, grid_lp_a = sg.butter(3, 5000 / (SR / 2), "low")
gridL = sg.lfilter(grid_lp_b, grid_lp_a, gridL)
gridR = sg.lfilter(grid_lp_b, grid_lp_a, gridR)
# the grid swells under the pilot then recedes -- dip ~t=110-125 s (1:54 drone)
grid_arc = ramp_env([0, 14, 60, 105, 115, 125, 150, 195, DUR],
                    [0.0, 0.10, 0.85, 0.90, 0.68, 0.74, 0.80, 0.20, 0.0])
L += 0.30 * grid_arc * gridL / gpk   # reduced from 0.34 to ease overall density
R += 0.30 * grid_arc * gridR / gpk
...
# pilot presence check: energy in a narrow band around 2437.5 Hz (alt: octave-down pilot)
pm_ = (freqs >= 2380) & (freqs <= 2500)
print(f"[QA] pilot band 2380-2500 Hz: {100*spec[pm_].sum()/tot:5.2f} % of energy")

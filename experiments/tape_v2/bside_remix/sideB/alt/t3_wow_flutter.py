...
OUT = "/Users/magnus/repos/cassette-ai/experiments/tape_v2/bside_remix/sideB/alt/t3.wav"
...
# ALT v1: tamed per listener feedback – gain halved (-6 dB: 0.014 -> 0.007) and
# a 4th-order Butterworth LP at 4 kHz softens the shrillness before it hits the bus.
pilot = np.sin(2 * np.pi * 4875.0 * t)
pilot = wow_resample(pilot, flutter_speed(0.8, depth_env=depth, seed_off=9))
_plt_b, _plt_a = sg.butter(4, 4000.0 / (SR / 2), "low")
pilot = sg.lfilter(_plt_b, _plt_a, pilot)
pgate = smooth_gate(14.0, DUR - 6.0, 16.0, 30.0)   # pilot is the last to leave
L += 0.007 * pgate * pilot
R += 0.007 * pgate * np.roll(pilot, SR // 5)

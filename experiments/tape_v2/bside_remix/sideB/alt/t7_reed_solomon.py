...
OUT = "/Users/magnus/repos/cassette-ai/experiments/tape_v2/bside_remix/sideB/alt/t7.wav"
...
def fill_prob(bar):
    ...
    # alt: early-entry — kick/hats seed the grid from bar 14 (sparse),
    # bass seeded from bar 26; the full partial-lock density still builds from bar 20.
    if bar < 14:
        return 0.0
    if bar < 20:
        return 0.30   # sparse early pulse: ~30% steps fire -> groove hint without full lock
    if bar < 40:
...
# alt: kick seeds the grid from bar 14 (~27.5 s), not bar 20 (~39 s),
# so the listener has rhythmic anchor ~12 s earlier.
kick_bars = range(14, 97)
for b in kick_bars:
    for s in (0, 4, 8, 12):
...
# --- HATS ---
# alt: hats also seed from bar 14 alongside kick for early texture.
for b in range(14, 97):
...
# alt: bass enters at bar 26 (~51 s) instead of bar 34 (~67 s);
# the groove/bass combo the listener loves arrives ~16 s earlier.
BASS_PAT = [(2, F2, 1.0, 0.21), (6, F2, 0.9, 0.21), (7, As2 * 0.9438, 0.6, 0.10),
            (10, Cs3, 0.95, 0.21), (14, E3, 0.9, 0.17), (15, F2, 0.7, 0.10)]
for b in range(26, 97):
...
def pat_full(j):       # dense data voice once locked
    ...
    if j % 2 == 1:
        # alt: was DATA0+0.205 = 0.575 s = step 4.68 (off-grid).
        # Snapped to 5*STEP = 0.6146 s (exact 16th-note step 5 within the 2-beat frame).
        # The background scrape events now land on-beat instead of floating between 16ths.
        ev.append((5 * STEP, 0.10, 0.75))
    if j % 4 == 2:
        ev.append((0.75 * FRAME_S, 0.14, 0.8))   # 6*STEP = 0.7375 s, already on grid
    return ev

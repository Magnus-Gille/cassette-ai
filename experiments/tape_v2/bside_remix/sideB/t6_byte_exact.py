#!/usr/bin/env python3
"""
B-SIDE Track 6 — "Byte-Exact"  (concept album, side B of the DOOM-on-cassette tape)

THE MOMENT OF CLEAN DECODE. 0 codewords failed. The CRC passes. This is the
emotional payoff: a major-lifted resolution of the 'Three Seventy-Five' theme,
the carrier grid retuned from its native F# (+23c) up into clean equal-temperament
F# MAJOR, the chord stacks ringing out perfectly STEADY — no flutter wobble now,
because the data came back byte-exact. That steadiness is the whole point: every
earlier track fought the wow/flutter; here the pitch is locked, rock-solid.

Every element is derived from / quotes the REAL data-transfer signal:

  * GROOVE / TEMPO : the d2x frame period (0.983333 s) = 2 beats -> 122.034 BPM, a
                     confident mid-tempo. A half-time heroic backbeat (61 BPM) rides
                     on top, so the 7:5-ish polyrhythm between the section families
                     is felt as anthem-vs-pulse.
  * REAL CHIRP     : the REAL global up-chirp sampled off the tape (capture 20.863 s)
                     opens the piece — "decode begins" — and seeds a resynth riser.
  * REAL TICKS     : the REAL frame-preamble ticks (capture 46.863 s + k*frame) are
                     the rim/clap of the groove, placed on the true frame grid.
  * REAL CRASH     : the REAL Schroeder multitone sounder splash (capture ~22/26 s)
                     is the triumphant crash on each chorus downbeat.
  * REAL GRID      : the literal N512 carrier grid (375 Hz harmonics, 750..9000 Hz)
                     retuned EXACTLY to clean ET F# major and rung out as sustained,
                     PERFECTLY STEADY chord stacks (the "ringing clean" payoff) plus a
                     shimmering high grid-arp.
  * REAL DQPSK     : a band-passed slice of the real d2x DQPSK capture, looped as a
                     faint "data is intact" hiss bed under the harmony.

THEME: the 'Three Seventy-Five' motif (the grid fundamental F#) stated minor-ish
earlier on the record now resolves to F# MAJOR. The recurring melodic "CRC-PASSES"
motif is a rising, affirmative 3-note figure (5 -> 7 -> 8, i.e. C# -> F -> F#-octave)
that lands clean on the tonic every time the checksum clears.

FORM (122.034 BPM, bar = 1.9667 s):
  intro(4)  verse1(8)  chorus1(8)  verse2(8)  CHORUS2(12 big)  outro(8)  = 48 bars
  ~= 94.4 s of bars + tail.  Repeated x2-ish density build to land ~210 s? No:
  to hit the ~210 s target we run a longer arrangement (see N_BARS below).

Output: sideB/t6.wav  (stereo 48 kHz 16-bit, peak <= -1 dBFS, RMS ~ -15 dBFS,
2 s fade-in / 4 s fade-out). Self-QA prints peak/RMS/duration, 10-band spectral
balance, onset rate (expect clear melodic mid-band energy + a steady mid onset rate).

Seed: 20260616 (logged below).
"""
import numpy as np
...
OUT = f"{BASE}/bside_remix/sideB/t6.wav"
...
bed_raw = bp(cslice(206.0, 10.0), 3500, 9000)
nx = int(0.3 * SR)
...
def lay_grid_ring(name, level):
    ...
    for i, ch in enumerate(seq):
        ...
        yl *= e; yr *= e
        m = max(np.max(np.abs(yl)), np.max(np.abs(yr))) + 1e-9
        add_st(lp(yl / m, 11000), lp(yr / m, 11000), t_of(bar), level)
...
# INTRO (6): real up-chirp (decode begins) + grid ring swelling in + bed
...
# CHORUS 2 (12, big): everything, theme up top, grid arp 16ths, double crash
b0, nb = sb("chorus2")
...
# CHORUS 3 (12): biggest "normal" chorus, full grid stacks, theme + octave double
b0, nb = sb("chorus3")
add(upchirp, t_of(b0) - 0.2, 0.30)
add_st(crashL, crashR, t_of(b0), 0.30)
...
# CHORUS 4 (16, BIGGEST): plagal amen lift (I-IV-I), full grid stacks ringing clean
...
# intent sanity checks: TRIUMPH track 6 -> clear melodic MID-BAND energy + steady mid-tempo groove
mid_energy = band_db[1000] + band_db[2000]     # melody/theme + grid live here
print(f"[QA intent] mid-band (1k+2k) sum = {mid_energy:+.1f} dB "
      f"-> expect clear melodic mid presence (not buried)")
...
# verify a confident groove pulse near the d2x frame / beat

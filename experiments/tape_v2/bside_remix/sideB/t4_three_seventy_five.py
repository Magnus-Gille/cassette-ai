...
OUT = f"{BASE}/bside_remix/sideB/t4.wav"
...
def render_voice(events, t0_s, level=0.16, transpose=0, glide_tau=0.030,
                 vib_hz=5.2, vib_depth=0.0035, timbre=(1.0, 0.33, 0.15)):
    ...
    f[0] = ftgt[0]
    ph = 2 * np.pi * np.cumsum(f) / SR
    vib = vib_depth * np.sin(2 * np.pi * vib_hz * np.arange(n) / SR) * np.linspace(0, 1, n) ** 0.3
    ph = ph * (1 + vib)
    h1, h3, h5 = timbre

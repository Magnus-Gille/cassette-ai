...
OUT = "/Users/magnus/repos/cassette-ai/experiments/tape_v2/bside_remix/sideB/t8.wav"
...
duck = env_bp(N, [(0, 1), (106.5, 1), (108.0, 0.16), (115.5, 0.16),
                  (117.5, 1.0), (DUR, 1)])
L *= duck
R *= duck

rev = norm(grab(N512_PRE0 + 9 * N512_T - 0.10, 9.0, fade=0.001), 0.085)
re_env = np.ones(len(rev))
n_fi, n_fo = int(0.6 * SR), int(1.6 * SR)
...
place(rev * re_env, np.roll(rev, 31) * re_env, 108.0, 1.0, 0.0)

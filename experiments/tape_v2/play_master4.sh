#!/bin/bash
# Play master4.wav at volume 75 with a live countdown + progress bar.
# Usage:  bash experiments/tape_v2/play_master4.sh
# Start your deck RECORDING first, then run this (it gives a 3s lead-in).

WAV="/Users/magnus/repos/cassette-ai/experiments/tape_v2/master4.wav"
[ -f "$WAV" ] || { echo "not found: $WAV"; exit 1; }

# duration in whole seconds (soundfile, with afinfo fallback)
...
echo "master4.wav — $((DUR/60))m $((DUR%60))s.  Make sure the deck is RECORDING."
...
echo "Now read the tape back via Voice Memos -> iCloud, then run m4_decode.py on the capture."

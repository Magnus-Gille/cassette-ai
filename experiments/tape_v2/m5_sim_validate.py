"""m5_sim_validate.py — push master5.wav through the FAITHFUL real_channel_sim
and decode with m5_decode, to predict the rate-ladder's real-tape behaviour
BEFORE recording.

Same approach as m4_sim_validate: run the exact master5.wav that would be laid to
cassette through real_channel_sim.real_channel (calibrated to our measured acoustic
loop), then decode with the SAME m5_decode pipeline. Byte-exact per rung is checked
against the sidecars. capture="master3" = clean tape, "master2" = AAC voicememo path.
"""
...
import m5_decode                  # noqa: E402

SR = 48_000


def run(capture: str, seeds, master=str(_HERE / "master5.wav")):
    ...
    for seed in seeds:
        ...
        out = m5_decode.decode(tmp, out_tag=tag, verbose=True)
...
def main():
    ...
    (_HERE / "results" / "m5_sim_validate_summary.json").write_text(
